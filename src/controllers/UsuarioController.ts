...
export class UsuarioController {
  ...
  async downloadPdf(req: Request, res: Response) {
    ...
    html +=
      "<tr><th>ID</th> <th>Nome</th> <th>E-mail</th> <th>CPF</th> <th>Telefone</th> <th>Endereço</th></tr>";
    ...
    res.append(
      "Content-Disposition",
      'attachment; filename="ListaClientes.pdf"'
    );
    res.send(pdfBuffer);
  }
  ...
  async exportCsv(req: Request, res: Response): Promise<Response> {
    ...
    let header = '"ID";"Nome";"E-mail";"CPF";"Telefone";"Endereço";"Cidade"\n';
    let csv = header;

    usuario.forEach((element) => {
      csv += `"${element.id}";"${element.nome}";"${element.email}";"${element.cpf}";"${element.telefone}";"${element.endereco}";"${element.cidade.nome}"\r`;
    });
  ...
  async login(req: Request, res: Response): Promise<Response> {
    ...
    let usuarioLogin = await Usuario.findOneBy({
      email: body.email,
      senha: body.senha,
    });
    let adminLogin = await Admin.findOneBy({
      email: body.email,
      senha: body.senha,
    });
